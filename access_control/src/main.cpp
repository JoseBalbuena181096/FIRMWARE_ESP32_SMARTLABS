...
const String serial_number = "SMART00009";
...
void codeForTask1(void *parameter)
{
	...
	for (;;)
	{
		bool find_stop = true;
		char char_now;
		while (Serial2.available() && find_stop) {
			
			char_now = char(Serial2.read()); 
			if (char_now  == '\t') {
				find_stop = false;
				continue;
			}
...
void setup() {
	...
	void access_screen(bool access) {
		...
		if (access) {
            ...
            display.println("HELLO " + user_name);
            display.println("");
			display.println("DEVICE TURN ON ");
            display.display();
			delay(5000);
			opening();
		}else{
            ...
            display.setTextColor(SH110X_WHITE);
            display.setCursor(0, 0);
            display.println("USER NOT FOUND, TRY AGAIN");
            display.display();
			delay(2500);
			iddle();
		}
	...
	void closing() {
        ...
        display.setCursor(0, 0);
        display.setTextSize(1);
        display.println("TURN OFF DEVICE ... ");
        display.display();
		delay(2500);
		iddle();
	...
	void sending() {
        ...
        display.setCursor(0, 0);
        display.setTextSize(1);
        display.println("WAIT SENDING TO THE  SERVER");
        display.display();
		delay(300);
		display.print(".");
        display.display();
	...
	void iddle(){
        ...
        display.setCursor(0, 0);
        display.println("PUT YOUR  TARGET ON READER");
        display.display();
	}
